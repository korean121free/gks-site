...
- [ ] 2단계 인증 **백업 코드**를 단체 보관 — **구글 것이 아직 없음**(2단계 인증 화면의 "복구 코드 → 백업 코드 받기"가 주황 느낌표). 구글이 뿌리라 이게 가장 급함. 지금은 사역자님 폰과 이 컴퓨터에만 열쇠가 있음
  - ⚠️ **백업 코드를 이 저장소에 넣지 말 것.** 공개 저장소입니다. 종이·단체 문서·비밀번호 관리자에만
  - ⚠️ 다운로드 폴더에 **GHM 것과 GKS 것이 섞입니다.** 파일 이름 앞에 `GKS-`를 붙여 구분할 것
