...
- [x] 2단계 인증 **백업 코드 발급·보관** — 구글·Cloudflare 둘 다 받아서 `비밀금고/` 에 보관 (2026-07-31)
  - 보관 위치는 `C:\GitHub\gks-site\비밀금고\` 이고, [.gitignore](.gitignore) 11번 줄 `비밀금고/` 로 깃에서 제외됨 (2026-07-31 확인). **파일 내용은 절대 커밋하지 말 것 — 공개 저장소입니다**
  - 이 폴더를 다른 데로 옮긴다면 반드시 `C:\GitHub` **바깥**으로. 저장소 안의 다른 폴더로 옮기면 위 보호가 사라짐
- [ ] 백업 코드를 **종이로 인쇄해 단체 보관** — 지금은 열쇠가 이 컴퓨터와 사역자님 폰뿐. 컴퓨터가 고장나면 79번 항목의 걱정이 그대로 살아남
